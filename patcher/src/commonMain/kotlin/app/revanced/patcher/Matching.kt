...
fun <T> afterAtLeast(steps: Int = 1, predicate: IndexedMatcherPredicate<T>) =
    after<T>(steps..steps) { lastMatchedIndex, currentIndex, setNextIndex ->
        predicate(lastMatchedIndex, currentIndex, setNextIndex)
    }

fun <T> afterAtLeast(steps: Int = 1, predicate: Predicate<T>) =
    after<T>(steps..Int.MAX_VALUE) { _, _, _ -> predicate() }

fun <T> afterAtMost(steps: Int = 1, predicate: IndexedMatcherPredicate<T>) =
    after<T>(1..steps) { lastMatchedIndex, currentIndex, setNextIndex ->
        predicate(lastMatchedIndex, currentIndex, setNextIndex)
    }

fun <T> afterAtMost(steps: Int = 1, predicate: Predicate<T>) = after<T>(1..steps) { _, _, _ -> predicate() }

fun <T> after(steps: Int = 1, predicate: IndexedMatcherPredicate<T>) =
    after<T>(steps..steps) { lastMatchedIndex, currentIndex, setNextIndex ->
        predicate(lastMatchedIndex, currentIndex, setNextIndex)
    }

fun <T> after(steps: Int = 1, predicate: Predicate<T>) = after<T>(steps..steps) { _, _, _ -> predicate() }
...
fun <T> unorderedAllOf(vararg predicates: IndexedMatcherPredicate<T>): Array<IndexedMatcherPredicate<T>> {
    ...
    return (0 until predicates.size).map<Int, IndexedMatcherPredicate<T>> { patternIndex ->
        predicate@{ lastMatchedIndex, currentIndex, setNextIndex ->
...
inline fun <reified T : Reference> reference(
    crossinline predicate: Predicate<T> = { true }
): IndexedMatcherPredicate<Instruction> = { _, _, _ -> (reference as? T)?.predicate() == true }
...
class MatchBuilder private constructor(
    ...
    fun match(classDef: ClassDef) = Match(
        context, classDef.firstMethodDeclarativelyOrNull { predicate() }, indices
    )
}
